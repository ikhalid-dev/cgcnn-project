...
2 & \texttt{31\_train\_\allowbreak{}joint} & matbench & Shared trunk, two heads; ratio
  ...
  & MAE $\log_{10}(K/G)$ $0.0993 \to 0.0887$ \\[2pt]
...
\textbf{One number is unresolved.} \texttt{31\_train\_joint.py}'s docstring
quotes the separate models' residual correlation as $+0.297$, while the
README's table reports separate $+0.263$ against joint $+0.297$. The two cannot
both be right. The table above therefore quotes round 2 by its
MAE~$\log_{10}(K/G)$ improvement, which is consistent across both sources.}
