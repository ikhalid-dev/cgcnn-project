...
the five points at which our approach departs from the paper, and a
methodological error --- training on the intersection of our structures with
the labelled benchmark rather than on the benchmark itself --- that cost a
factor of two in accuracy before it was identified.
\end{abstract}
...
% =====================================================================
\section{Data, and a mistake worth recording}
...
\subsection{The mistake}

Our first approach trained on those 278 overlapping crystals. It reached a test
MAE of $0.152$ $\log_{10}$(GPa) --- more than double the paper's --- and
overfitted badly, with training error less than half of test error.

\keybox{The 1{,}213 structures were never the limit; the labels were}{Treating
our CIF collection as ``the dataset'' and intersecting it with the labelled
benchmark discarded 97\% of the available labels. The correct framing inverts
the roles: \textbf{train on all 10{,}987 matbench crystals, and treat the 1{,}213
CIFs purely as a prediction set.} That is what the paper does, and it more than
halved the error. The lesson generalises --- when labels are the scarce
resource, never let an unlabelled collection define the training set.}

\begin{table}[h]
\centering
\caption{Effect of the reframing. Same architecture and code; only the training
set differs.}
\begin{tabular}{lccc}
\toprule
Training set & Test MAE $\log_{10}$ & Test $R^2$ & Train/test gap \\
\midrule
278 crystals (the intersection) & 0.152 & 0.57 & $0.078 \rightarrow 0.152$ \\
10{,}987 crystals (all of matbench) & \textbf{0.070} & \textbf{0.882}
  & $0.037 \rightarrow 0.070$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent
Note the second column of gaps. On 278 crystals the model scored twice as well
on data it had seen --- the signature of memorisation. With 40$\times$ more data
the gap collapses to ordinary generalisation error. The overfitting was a
data-quantity problem, not a regularisation one.
