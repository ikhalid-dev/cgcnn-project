#!/usr/bin/env python3
"""
Push the direct-kappa (no Slack) kernel to Kaggle, watch it, and pull it back.
================================================================================

    python kaggle/build_direct_kappa_kernel.py     # build first
...
STAGING_DIR = os.path.join(PROJECT_ROOT, "kaggle", "output_direct_kappa")
...
def kernel_id():
    ...
    if not os.path.exists(path):
        sys.exit("No kernel-metadata.json - run "
                 "`python kaggle/build_direct_kappa_kernel.py` first.")
...
def fetch(kid):
    ...
    if not copied:
        ...
        print("Next:  python direct_kappa_no_slack/02_compare_direct_vs_slack.py")
...
def main():
    ...
    if args.log:
        ...
        log = os.path.join(STAGING_DIR, "pink_direct.log")
