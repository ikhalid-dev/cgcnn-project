#!/usr/bin/env python3
"""
Push the AFLOW matbench-recipe kernel to Kaggle, watch it, and pull it back.
================================================================================

    python kaggle/build_aflow_recipe_kernel.py     # build first
...
STAGING_DIR = os.path.join(PROJECT_ROOT, "kaggle", "output_aflow_recipe")
...
def kernel_id():
    ...
    if not os.path.exists(path):
        sys.exit("No kernel-metadata.json - run "
                 "`python kaggle/build_aflow_recipe_kernel.py` first.")
...
def fetch(kid):
    ...
    if not copied:
        ...
        print("Next:  python direct_kappa_no_slack/53_compare_aflow_recipe.py")
...
def main():
    ...
    if args.log:
        ...
        log = os.path.join(STAGING_DIR, "pink_aflow_recipe.log")
