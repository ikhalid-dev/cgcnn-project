...
\vspace{3pt}
\textbf{Start with MoBr$_5$Cl} --- 7 atoms, $Cm$: ALIGNN 0.028 vs round 9's
4.524, a factor of 162.}
\end{frame}
...
\begin{frame}
\SlideHeader{29c \textperiodcentered\ NEXT}{The shortlist, as it stands after step 64}
...
the six adjudication materials without any DFT: round 9's predicted $K$ exceeds
\emph{every} real measured $K$ for those chemistries, while ALIGNN's sits inside
the real range. The four negative controls stay as controls.

\vspace{3pt}
\textbf{$\kappa$ is ALIGNN's}, not an average of the four --- averaging across a
refuted model produces a number no model believes. \textbf{Spread} is
$\kappa_{\max}/\kappa_{\min}$ across the four, and it separates the arms by
itself: these eight span 1.3--2.9$\times$, the six dropped ones 89--162$\times$.

\vspace{3pt}
\textcolor{Muted}{Source: \texttt{results/cgcnn/66\_dft\_queue\_slide.csv}
(full table with status and reason in \texttt{66\_dft\_queue.csv}).
Start with \textbf{Rb$_4$MnBeTe$_4$}, the lowest predicted.}}
\end{frame}
