...
if [ ! -f data_full/graphs.pt ]; then
    ...
    python scripts/cgcnn/01b_prepare_full_dataset.py
else
...
# on. See scripts/cgcnn/05_ensemble.py, which refuses to combine mismatched members.
if [ "$MODE" != "--predict" ]; then
    ...
    for TARGET in K_VRH G_VRH; do
        for SPEC in "${TARGET}_full 42 3" "${TARGET}_s1 1 4" "${TARGET}_s2 2 3"; do
            ...
            python -u scripts/cgcnn/02_train.py \
...
for TARGET in K_VRH G_VRH; do
    python scripts/cgcnn/03_evaluate.py --target "$TARGET" --data-dir data_full \
        --tag "${TARGET}_full"
    python scripts/cgcnn/05_ensemble.py --target "$TARGET" --data-dir data_full \
...
echo "=================================================================="
echo " Done."
echo "   results/cgcnn/RESULTS.md                  headline numbers"
echo "   results/cgcnn/metrics_summary.csv         every model, every split"
echo "   results/cgcnn/pink_moduli_predictions.csv K and G for 1,213 crystals"
echo "   results/cgcnn/parity_*_ens.png            predicted vs DFT"
echo "=================================================================="
